...
Neutral, unbranded `.docx` style pack. The default document style pack for [[ai-project-kickoff]] (see [[architecture]] for the delegation pattern).
...
## How it's invoked
Not a slash command — it's a Skill (`skills/generic-docx/SKILL.md`) that other agents (chiefly `ai-project-kickoff`'s `gate-renderer`) invoke via the Skill tool. The actual rendering logic is a bundled Python helper: `scripts/generic_docx.py`, built on `python-docx`. Requires Python 3 + `pip install python-docx`; Graphviz is optional (diagrams).
